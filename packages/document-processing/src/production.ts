...
export class QpdfInspector {
  ...
  async inspect(bytes: Uint8Array, limits: { readonly maximumBytes: number; readonly maximumPages: number }): Promise<PdfInspectionReport> {
    ...
    try {
      ...
      const pages = Number((await run(this.command, ['--show-npages', file], this.timeoutMs)).stdout.trim());
      if (!Number.isSafeInteger(pages) || pages < 1 || pages > limits.maximumPages) throw new Error('DOCUMENT_PAGE_LIMIT_EXCEEDED');
      const encryptionOutput = (await run(this.command, ['--show-encryption', file], this.timeoutMs)).stdout;
      const encrypted = !/File is not encrypted/i.test(encryptionOutput);
      const jsonOutput = (await run(this.command, ['--json', file], this.timeoutMs, 25 * 1024 * 1024)).stdout;
...
export class VeraPdfRestClient {
  ...
  async validatePdfA2b(bytes: Uint8Array): Promise<PdfAValidationReport> {
    ...
    const response = await timedFetch(this.http, `${this.baseUrl}${this.validatePath}`, { method: 'POST', headers: { accept: 'application/json, application/xml;q=0.9' }, body: form }, this.timeoutMs);
    ...
    const version = response.headers.get('x-verapdf-version') ?? await this.version().catch(() => 'unknown');
    return { engine: 'veraPDF', engineVersion: version, profile: 'PDF/A-2b', compliant, rawReport, rawReportContentType: contentType };
  }
  async version(): Promise<string> {
    const response = await timedFetch(this.http, `${this.baseUrl}/api/info`, { headers: { accept: 'application/json, application/xml' } }, 5_000);
