...
- **PENDING_ADOPTION** — Styrdokumentet eller rutinen är utkastad och ligger i repot, men inte antagen. Räknas som ouppfyllt: ett utkast leverantören själv skrivit är inte ett beslut. Flyttas till PASS när antagandet är registrerat med datum, beslutsfattare och nästa översyn.
...
| Lösning | apps/workers/src/pades-handlers.ts (stegordning och bindning), packages/pades (antagningsgrind), migrations/data/0021_pades_signature_chain_guards.sql (databasens guards), services/signservice (backendval och fail-closed). |
| Kodevidens | apps/workers/src/pades-handlers.ts; packages/pades/src/index.ts; migrations/data/0021_pades_signature_chain_guards.sql; services/signservice/src/main/java/se/kommunsign/signservice/SigningEngineFactory.java; services/signservice/src/main/java/se/kommunsign/signservice/BlockedSigningEngine.java; docs/architecture/adr/0003-signing-backend-dependency-policy.md |
| Verifiering | tests/run.mjs: PAdES-antagningstester. tests/sql/pades-signature-chain.sql kör guardsen mot databasen. verify:e2e:signing skapar en riktig signatur med test-CA och låter validation-service underkänna en ändrad byte. |
| Status | BLOCKED_EXTERNAL |
| Blockerare | CA-utfärdat signeringscertifikat för organisationen, HSM eller fjärr-QSCD för nyckelskydd, samt TSA-avtal för RFC 3161-tidsstämplar. Ingen av dessa kan tillhandahållas av kod. |
| Bedömningskälla | Override 2026-08-21: build-requirement-matrix.mjs now fails when an assessment cites a path that does not resolve on disk. That check found eight dead pointers across six requirements: five rotted when packages were deleted on 2026-08-20, and three named a Java file that has never existed under that name. Correcting them exposed two claims that were wrong independently of the pointer, and those are corrected here too. Original requirements remain immutable in requirements.json. A second pass extended the reachability gate to module granularity, which removed packages/signing-engine/src/index.ts and the generic external-signing adapter, and made packages/crypto/src/key-rotation.ts reachable by giving the AES-GCM adapter a real key ring. En tredje pass byggde klart de tre krav som stod under PASS: F007 (profilen når portalen), F011 (bilagan binds in utan att signeras) och 2028 (skyddade personuppgifter fick datakälla, anropare per utflödeskanal och guards i databasen). Kraven flyttas endast där varje del finns och är verifierad; för 2028 står de två kanaler som saknar väg in i produkten kvar som gap i stället för att räknas som uppfyllda. |
...
| Typ | SKA |
| Kategori | Funktion |
| Nuläge | Tenantens grafiska profil valideras och normaliseras när den skrivs (validateAndNormalizeBranding avvisar otillåtna färgvärden, orimliga namn och osäkra logotyp-URL:er och härleder läsbar textfärg ur bakgrunden med contrastRatio), lagras per tenant i control.tenant_branding med versionshistorik — och slår nu igenom i gränssnittet. Provisioneringen aktiverar den seedade versionen, som tidigare skrevs med active=false så att ingen tenant hade någon aktiv profil alls. activeBranding i public-signing-repository löser värdnamn till tenant till aktiv version och lämnar den normaliserade profilen med signeringsinbjudan, och signerportalens applyBranding sätter --brand, --brand-dark, --brand-text och --accent-text via documentElement.style.setProperty. Det sista är avsiktligt: portalerna kör under strikt CSP där inline style-attribut inte tillåts. |
| Gap | Ingen. |
| Lösning | packages/branding, apps/api/src/production-adapters/postgres/provisioning-repository.ts (aktiverar den seedade versionen och normaliserar den), apps/api/src/production-adapters/postgres/public-signing-repository.ts (activeBranding), apps/signer-portal/public/app.js (applyBranding). |
| Kodevidens | packages/branding/src/index.ts; apps/api/src/production-adapters/postgres/provisioning-repository.ts; apps/api/src/production-adapters/postgres/public-signing-repository.ts; apps/signer-portal/public/app.js |
| Verifiering | scripts/e2e-application-chain.mjs: steget där undertecknaren öppnar inbjudan kontrollerar att sidan renderas med tenantens färg (branded #174a7e) mot körande tjänster. tests/security.mjs täcker validering och avvisning av otillåtna värden. |
| Status | PASS |
| Bedömningskälla | Override 2026-08-21: build-requirement-matrix.mjs now fails when an assessment cites a path that does not resolve on disk. That check found eight dead pointers across six requirements: five rotted when packages were deleted on 2026-08-20, and three named a Java file that has never existed under that name. Correcting them exposed two claims that were wrong independently of the pointer, and those are corrected here too. Original requirements remain immutable in requirements.json. A second pass extended the reachability gate to module granularity, which removed packages/signing-engine/src/index.ts and the generic external-signing adapter, and made packages/crypto/src/key-rotation.ts reachable by giving the AES-GCM adapter a real key ring. En tredje pass byggde klart de tre krav som stod under PASS: F007 (profilen når portalen), F011 (bilagan binds in utan att signeras) och 2028 (skyddade personuppgifter fick datakälla, anropare per utflödeskanal och guards i databasen). Kraven flyttas endast där varje del finns och är verifierad; för 2028 står de två kanaler som saknar väg in i produkten kvar som gap i stället för att räknas som uppfyllda. |
...
| Gap | Ingen. |
| Lösning | migrations/data/0036_signing_turn_predicate.sql, apps/api/src/production-adapters/postgres/public-signing-repository.ts (loadStartInput), apps/workers/src/signing-groups.ts, app.signers.signing_order. |
| Kodevidens | migrations/data/0036_signing_turn_predicate.sql; apps/api/src/production-adapters/postgres/public-signing-repository.ts; apps/workers/src/signing-groups.ts; tests/sql/signing-turn.sql |
...
| Gap | Ingen. |
| Lösning | migrations/data/0036_signing_turn_predicate.sql, app.signers.signing_order, apps/api/src/production-adapters/postgres/public-signing-repository.ts (decline), migrations/data/0009 (completion guards). |
| Kodevidens | migrations/data/0036_signing_turn_predicate.sql; apps/api/src/production-adapters/postgres/public-signing-repository.ts; migrations/data/0009_integrity_and_worker_recovery.sql; tests/sql/signing-turn.sql |
...
| Typ | SKA |
| Kategori | Funktion |
| Nuläge | Bilagor binds in utan att signeras. app.signing_intent_documents bär sedan migrations/data/0040 en document_role, en ögonblicksbild av dokumentets roll när intentet skapades. Utskicksvägen tar med samtliga dokument i intentet och därmed i bindningsmaterialet, så en bilaga som byts ut efter att intentet skapades upptäcks på sin digest — men completion-guarden assert_signer_signature_evidence kräver validerad signatur endast för document_role='signable'. Ett intent som bara innehåller bilagor avvisas, eftersom det inte finns något att underteckna. loadStartInput väljer första signerbara dokumentet som first_document_version_id. |
| Gap | Ingen. |
| Lösning | migrations/data/0018_document_attachments.sql (document_role på dokumentet), migrations/data/0040_attachments_bound_not_signed.sql (document_role på intentet och den smalare guarden), apps/api/src/production-adapters/postgres/data-database.ts (send), apps/api/src/production-adapters/postgres/public-signing-repository.ts (loadStartInput). |
| Kodevidens | migrations/data/0040_attachments_bound_not_signed.sql; apps/api/src/production-adapters/postgres/data-database.ts; migrations/data/0021_pades_signature_chain_guards.sql |
| Verifiering | tests/sql/pades-signature-chain.sql: ett ärende med bilaga kan slutföras utan signatur på bilagan, och vägras när bilagans digest ändrats efter att intentet skapades. scripts/e2e-application-chain.mjs rapporterar 'attachment bound unsigned' mot körande tjänster. |
| Status | PASS |
| Bedömningskälla | Override 2026-08-21: build-requirement-matrix.mjs now fails when an assessment cites a path that does not resolve on disk. That check found eight dead pointers across six requirements: five rotted when packages were deleted on 2026-08-20, and three named a Java file that has never existed under that name. Correcting them exposed two claims that were wrong independently of the pointer, and those are corrected here too. Original requirements remain immutable in requirements.json. A second pass extended the reachability gate to module granularity, which removed packages/signing-engine/src/index.ts and the generic external-signing adapter, and made packages/crypto/src/key-rotation.ts reachable by giving the AES-GCM adapter a real key ring. En tredje pass byggde klart de tre krav som stod under PASS: F007 (profilen når portalen), F011 (bilagan binds in utan att signeras) och 2028 (skyddade personuppgifter fick datakälla, anropare per utflödeskanal och guards i databasen). Kraven flyttas endast där varje del finns och är verifierad; för 2028 står de två kanaler som saknar väg in i produkten kvar som gap i stället för att räknas som uppfyllda. |
...
| Lösning | apps/workers/src/production-handlers.ts (handleReminder), migrations/data/0036_signing_turn_predicate.sql, apps/api/src/production-adapters/postgres/data-database.ts (remind). |
| Kodevidens | apps/workers/src/production-handlers.ts; migrations/data/0036_signing_turn_predicate.sql; tests/sql/signing-turn.sql |
...
| Lösning | packages/document-processing (PDF/A-kanonisering och Office-konvertering), apps/workers/src/pades-handlers.ts (inkrementell signering och leveransartefakt), packages/archive (PDF/A-krav vid export). |
| Kodevidens | packages/document-processing/src/office-ingestion.ts; apps/workers/src/pades-handlers.ts; packages/archive/src/index.ts |
| Verifiering | tests/run.mjs: arkivtest som vägrar dokument utan verifierad PDF/A-profil, samt Office-ingestionstest. verify:e2e:application kör kanonisering och profilvalidering mot körande Gotenberg och veraPDF. |
| Status | BLOCKED_EXTERNAL |
| Blockerare | Samma som F001: CA-utfärdat signeringscertifikat, HSM eller fjärr-QSCD, och TSA-avtal. |
| Bedömningskälla | Override 2026-08-21: build-requirement-matrix.mjs now fails when an assessment cites a path that does not resolve on disk. That check found eight dead pointers across six requirements: five rotted when packages were deleted on 2026-08-20, and three named a Java file that has never existed under that name. Correcting them exposed two claims that were wrong independently of the pointer, and those are corrected here too. Original requirements remain immutable in requirements.json. A second pass extended the reachability gate to module granularity, which removed packages/signing-engine/src/index.ts and the generic external-signing adapter, and made packages/crypto/src/key-rotation.ts reachable by giving the AES-GCM adapter a real key ring. En tredje pass byggde klart de tre krav som stod under PASS: F007 (profilen når portalen), F011 (bilagan binds in utan att signeras) och 2028 (skyddade personuppgifter fick datakälla, anropare per utflödeskanal och guards i databasen). Kraven flyttas endast där varje del finns och är verifierad; för 2028 står de två kanaler som saknar väg in i produkten kvar som gap i stället för att räknas som uppfyllda. |
...
| Nuläge | Styrdokumentet är utkastat och ligger i repot med tomt antagandeblock. |
| Gap | Utkast finns; det som återstår står i blockeraren. |
| Lösning | docs/governance/INCIDENTHANTERING.md |
...
| Nuläge | Skyddade personuppgifter hanteras av tjänsten, inte bara av ett bibliotek. app.signers.protection_level bär Skatteverkets tre nivåer var för sig med CHECK mot fyra tillåtna värden; nivån anges av kunden när undertecknaren läggs till, eftersom Kommunsign inte har något folkbokföringsregister och inte ska låtsas ha det. Ett okänt värde normaliseras av normaliseProtectionLevel till den strängaste nivån, så att ett datafel eller en ny kod hos Skatteverket inte tyst tar bort skyddet. Menprövningen registreras i app.protected_identity_assessments med grund, beslutsfattare och utgång: grunden måste vara verklig text, utgången måste ligga efter beslutet och högst tolv timmar fram, raden är immutabel utom för återkallelse, och databasen vägrar registrera en menprövning för en oskyddad eller fingerad identitet. decideDisclosure avgör per utflödeskanal vilka fält som får lämna systemet, och anropas nu från de kanaler som finns: e-postens ämnesrad genom assertSubjectLineIsSafe, e-postkroppen och ärendevyn (SCREEN_AUTHORISED, som maskerar namn till 'Skyddad identitet'), evidenspaketet, och arkivexportens maskerade identifierare. Auditloggen är en kanal som bevakas i databasen: audit.append_event vägrar en payload vars nycklar namnger ett identifierande fält, vilket täcker samtliga tio anropsställen på en gång och gör regeln kontrollerad i stället för respekterad. Vid skyddad folkbokföring lämnas adressen aldrig ut medan namnet kvarstår i evidenspaketet, så att signaturen förblir bevisbar; vid fingerade personuppgifter vägrar evidenspaketet byggas i stället för att tyst byggas utan den evidens det påstår sig bära. |
| Gap | Två av de tretton kanalerna i modellen har ingen väg in i produkten i dag och är därför en arkitektonisk gräns snarare än en kontroll som körs: SEARCH_RESULT, eftersom det inte finns någon sökning på personer i tenant-API:t, och SUPPORT_TOOLING, eftersom plattformsplanet saknar route till tenantens undertecknare. isSearchable och assertSupportAccess är därmed provade som regler men har ingen anropare. Det skrivs ut här i stället för att påstås som en teknisk kontroll. |
| Lösning | packages/protected-identity (regler och menprövningens uppslagning), migrations/data/0041_protected_personal_data.sql (nivå och menprövning med guards), migrations/data/0042_audit_payload_carries_no_identity.sql (auditloggens kanalgrind), apps/api/src/router.ts och data-database.ts (nivå in, menprövning registrerad bakom protected-identity:assess, maskerad ärendevy), apps/workers/src/production-handlers.ts (ämnesrad, e-postkropp, evidenspaket), apps/workers/src/archive-handlers.ts (arkivexport). |
| Kodevidens | packages/protected-identity/src/index.ts; packages/protected-identity/src/store.ts; migrations/data/0041_protected_personal_data.sql; migrations/data/0042_audit_payload_carries_no_identity.sql; apps/workers/src/production-handlers.ts; apps/workers/src/archive-handlers.ts |
| Verifiering | tests/run.mjs: tre tester för reglerna (kanaler som lämnar åtkomstkontrollen, maskering per skyddsnivå inklusive samtliga kanaler för fingerade uppgifter, supportåtkomst per person med utgång). tests/sql/protected-identity.sql: nivåns CHECK, menprövningens guards och immutabilitet, samt att auditloggen vägrar ett identifierande fält även nästlat. scripts/e2e-application-chain.mjs: ett ärende med en undertecknare med skyddad folkbokföring går hela kedjan, och ärendevyn maskerar namnet medan inbjudans ämnesrad och kropp inte bär det. |
| Status | PASS |
| Bedömningskälla | Override 2026-08-21: build-requirement-matrix.mjs now fails when an assessment cites a path that does not resolve on disk. That check found eight dead pointers across six requirements: five rotted when packages were deleted on 2026-08-20, and three named a Java file that has never existed under that name. Correcting them exposed two claims that were wrong independently of the pointer, and those are corrected here too. Original requirements remain immutable in requirements.json. A second pass extended the reachability gate to module granularity, which removed packages/signing-engine/src/index.ts and the generic external-signing adapter, and made packages/crypto/src/key-rotation.ts reachable by giving the AES-GCM adapter a real key ring. En tredje pass byggde klart de tre krav som stod under PASS: F007 (profilen når portalen), F011 (bilagan binds in utan att signeras) och 2028 (skyddade personuppgifter fick datakälla, anropare per utflödeskanal och guards i databasen). Kraven flyttas endast där varje del finns och är verifierad; för 2028 står de två kanaler som saknar väg in i produkten kvar som gap i stället för att räknas som uppfyllda. |
...
| Nuläge | Styrdokumentet är utkastat och ligger i repot med tomt antagandeblock. |
| Gap | Utkast finns; det som återstår står i blockeraren. |
| Lösning | docs/governance/UNDERBITRADEN.md |
...
| Område | Underleveranrör |
| Nuläge | Styrdokumentet är utkastat och ligger i repot med tomt antagandeblock. |
| Gap | Antagandet återstår: datum, beslutsfattare, dokumentägare och nästa översyn ska registreras, och kravet flyttas då till PASS i en daterad override. |
| Lösning | docs/governance/UNDERBITRADEN.md |
| Kodevidens | docs/governance/UNDERBITRADEN.md; docs/governance/README.md |
| Verifiering | Dokumentet finns i repot. Antagandeblocket är tomt, vilket är hela poängen med statusen: ett utkast leverantören själv skrivit är inte ett beslut. |
...
| Nuläge | Paketets METS sip.xml valideras nu mot Riksarkivets publicerade schemauppsättning, av en schemaprocessor i stället för av koden som skriver strukturen. Uppsättningen är buntad i validation-service med hashar i PROVENANCE.txt — en konformitetskontroll som hämtar scheman över nätet är inte reproducerbar. Att slå på valideringen hittade två verkliga profilbrott som "följer profilen vid inspektion" hade missat: altRecordID är obligatoriskt i profilen och saknades helt, och ID på amdSec krävs där METS-kärnan gör det valfritt. Båda är rättade. FGS_CONFORMANCE_STATUS håller nu två skilda påståenden isär: publishedSchemaValidated=true och receivingArchiveSchemaValidated=false. |
| Gap | Ingen kodbrist. Kvar är mottagande arkivs version och lokala utökningar. |
| Lösning | services/validation-service/.../FgsPackageValidator.java (schemavalidering mot buntad uppsättning, LSResourceResolver som aldrig når nätet), /v1/validate/fgs, packages/archive/src/fgs.ts (altRecordID, amdSec ID, delad konformitetsstatus), scripts/verify-fgs-package.mjs. |
| Kodevidens | packages/archive/src/fgs.ts; services/validation-service/src/main/java/se/kommunsign/validation/FgsPackageValidator.java; scripts/verify-fgs-package.mjs |
...
| Nuläge | Styrdokumentet är utkastat och ligger i repot med tomt antagandeblock. |
| Gap | Antagandet återstår: datum, beslutsfattare, dokumentägare och nästa översyn ska registreras, och kravet flyttas då till PASS i en daterad override. |
| Lösning | docs/governance/INFORMATIONSSAKERHETSPOLICY.md |
...
| Nuläge | Styrdokumentet är utkastat och ligger i repot med tomt antagandeblock. |
| Gap | Antagandet återstår: datum, beslutsfattare, dokumentägare och nästa översyn ska registreras, och kravet flyttas då till PASS i en daterad override. |
| Lösning | docs/governance/PERSONALSAKERHET.md |
...
| Nuläge | Styrdokumentet är utkastat och ligger i repot med tomt antagandeblock. |
| Gap | Utkast finns; det som återstår står i blockeraren. |
| Lösning | docs/governance/RISKHANTERING.md |
| Kodevidens | docs/governance/RISKHANTERING.md; docs/governance/README.md |
| Verifiering | Dokumentet finns i repot. Antagandeblocket är tomt, vilket är hela poängen med statusen: ett utkast leverantören själv skrivit är inte ett beslut. |
| Status | BLOCKED_EXTERNAL |
...
| Nuläge | Nyckelrotation går att genomföra utan att göra data oläsbar. Kryptotjänsten håller en nyckelring i stället för en nyckel: varje kuvert bär versionen det skrevs under, ny text skrivs under den aktiva versionen, och läsning sker under den version kuvertet namnger så länge den versionen inte är pensionerad. Det är runbookens dual read-steg, och det fungerar nu i kod. En pensionerad version vägrar dekryptera — en pensionerad nyckel som ändå läser är inte pensionerad. Nio tabeller bär key_version, så frågan vilka rader som fortfarande ligger på den gamla nyckeln går att besvara, och databasen vägrar markera en rotation verifierad medan någon kolumn rapporterar utestående rader. |
| Gap | Blindindex versioneras inte i kuvert, eftersom ett index är en naken digest utan kuvert. Vilken nyckel som skapade det står i radens key_version, och rotation av ett blindindex kräver att samtliga index för en uppslagning räknas om samtidigt. Själva omkrypteringen är en operatörsåtgärd enligt runbooken; det finns inget jobb som kör den. |
| Lösning | apps/api/src/adapters/aes-gcm-sensitive-data.ts (nyckelring, versionsbyte i kuvertet), packages/crypto/src/key-rotation.ts (tillståndsmaskin och läs-/skrivregler), migrations/data/0029_key_rotation_backfill.sql, docs/runbooks/KEY_ROTATION.md. |
| Kodevidens | apps/api/src/adapters/aes-gcm-sensitive-data.ts; packages/crypto/src/key-rotation.ts; migrations/data/0029_key_rotation_backfill.sql; docs/runbooks/KEY_ROTATION.md |
| Verifiering | tests/run.mjs: rotationstest som skriver under version 1, aktiverar version 2, läser den gamla raden, skriver en ny under version 2, och sedan ser den gamla vägras när version 1 pensionerats. tests/sql/key-rotation.sql kör tillståndsmaskinen mot databasen. |
| Status | PASS |
| Bedömningskälla | Override 2026-08-21: build-requirement-matrix.mjs now fails when an assessment cites a path that does not resolve on disk. That check found eight dead pointers across six requirements: five rotted when packages were deleted on 2026-08-20, and three named a Java file that has never existed under that name. Correcting them exposed two claims that were wrong independently of the pointer, and those are corrected here too. Original requirements remain immutable in requirements.json. A second pass extended the reachability gate to module granularity, which removed packages/signing-engine/src/index.ts and the generic external-signing adapter, and made packages/crypto/src/key-rotation.ts reachable by giving the AES-GCM adapter a real key ring. En tredje pass byggde klart de tre krav som stod under PASS: F007 (profilen når portalen), F011 (bilagan binds in utan att signeras) och 2028 (skyddade personuppgifter fick datakälla, anropare per utflödeskanal och guards i databasen). Kraven flyttas endast där varje del finns och är verifierad; för 2028 står de två kanaler som saknar väg in i produkten kvar som gap i stället för att räknas som uppfyllda. |
...
| Nuläge | Styrdokumentet är utkastat och ligger i repot med tomt antagandeblock. |
| Gap | Utkast finns; det som återstår står i blockeraren. |
| Lösning | docs/governance/REVISION.md |
| Kodevidens | docs/governance/REVISION.md; docs/governance/README.md |
| Verifiering | Dokumentet finns i repot. Antagandeblocket är tomt, vilket är hela poängen med statusen: ett utkast leverantören själv skrivit är inte ett beslut. |
| Status | BLOCKED_EXTERNAL |
