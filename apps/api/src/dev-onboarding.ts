...
export const devOnboardingRepository: OnboardingRepository = {
  ...
  async approve(context,applicationId,input:DecisionInput,key,payloadHash){return idempotent(applicationId,'platform:approve',key,payloadHash,()=>{const current=requireApplication(applicationId);const risk=maximumRisk(applicationId);if(risk==='high'||risk==='critical'){if(!input.secondApproverId)throw new Error('TWO_PERSON_APPROVAL_REQUIRED');assertDistinctApprovers(context.subjectId,input.secondApproverId);}const updated=transition(current,'approved',context.subjectId);addAudit(applicationId,context.subjectId,'onboarding.decision.approved',{reason:input.reason,conditions:input.conditions??[],risk,secondApproverId:input.secondApproverId??null});return updated;});},
