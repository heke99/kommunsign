...
import {
  ...
  formatApplicationReference, isApplicationApprovedOrLater, isApplicationDecisionPending,
  verifyEmailToken, type EmailVerificationRecord,
...
export const devOnboardingRepository: OnboardingRepository = {
  ...
  async approve(context,applicationId,input:DecisionInput,key,payloadHash){return idempotent(applicationId,'platform:approve',key,payloadHash,()=>{const current=requireApplication(applicationId);if(isApplicationApprovedOrLater(current.status))return current;if(!isApplicationDecisionPending(current.status))throw new Error(['rejected','withdrawn','archived'].includes(current.status)?'APPLICATION_ALREADY_CLOSED':'INVALID_APPLICATION_STATE_TRANSITION');const risk=maximumRisk(applicationId);if(risk==='high'||risk==='critical'){if(!input.secondApproverId)throw new Error('TWO_PERSON_APPROVAL_REQUIRED');assertDistinctApprovers(context.subjectId,input.secondApproverId);}const updated=transition(current,'approved',context.subjectId);addAudit(applicationId,context.subjectId,'onboarding.decision.approved',{previousStatus:current.status,reason:input.reason,conditions:input.conditions??[],risk,secondApproverId:input.secondApproverId??null});return updated;});},
  async reject(context,applicationId,input:DecisionInput,key,payloadHash){return idempotent(applicationId,'platform:reject',key,payloadHash,()=>{const current=requireApplication(applicationId);if(current.status==='rejected')return current;if(!isApplicationDecisionPending(current.status)&&current.status!=='provisioning_failed')throw new Error(['withdrawn','archived'].includes(current.status)?'APPLICATION_ALREADY_CLOSED':'INVALID_APPLICATION_STATE_TRANSITION');const updated=transition(current,'rejected',context.subjectId);addAudit(applicationId,context.subjectId,'onboarding.decision.rejected',{previousStatus:current.status,reason:input.reason});return updated;});},
  async provision(context,applicationId,key,payloadHash){return idempotent(applicationId,'platform:provision',key,payloadHash,()=>{let app=requireApplication(applicationId);const existing=[...provisioning.values()].find((item)=>item.applicationId===applicationId);if(existing){if(app.status==='approved')app=transition(app,'provisioning',context.subjectId);if(existing.status==='completed'){if(app.status==='provisioning')saveApplication(transition(app,'onboarding',context.subjectId));return existing;}if(['failed','partially_completed','retry_scheduled','waiting_for_external_dependency'].includes(existing.status)){if(app.status==='provisioning_failed')app=transition(app,'provisioning',context.subjectId);if(app.status!=='provisioning')throw new Error('INVALID_APPLICATION_STATE_TRANSITION');const {blockingCode:_blockingCode,...existingWithoutBlocking}=existing;const updated:ProvisioningRequestView={...existingWithoutBlocking,status:'completed',currentStep:'onboarding_checklist_created',updatedAt:now()};provisioning.set(existing.id,updated);const tenantId=existing.tenantId??app.tenantId??crypto.randomUUID();const updatedApp=transition(saveApplication({...app,tenantId}),'onboarding',context.subjectId);saveApplication(updatedApp);platformOrganizations.set(applicationId,{applicationId,tenantId,provisioningRequestId:existing.id,legalName:updatedApp.organizationName,organizationNumber:updatedApp.organizationNumber,organizationType:updatedApp.organizationType,applicationStatus:updatedApp.status,tenantStatus:'onboarding',provisioningStatus:updated.status,...(updated.currentStep?{currentStep:updated.currentStep}:{}),primaryHostname:`${updatedApp.organizationName.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'').slice(0,40)||'organisation'}.kommunsign.se`,domainReady:true,primaryAdminEmail:updatedApp.primaryEmail,primaryAdminName:updatedApp.primaryContactName,primaryAdminTitle:updatedApp.primaryContactTitle,createdAt:updatedApp.createdAt});return updated;}return existing;}if(app.status!=='approved'&&app.status!=='provisioning_failed')throw new Error('INVALID_APPLICATION_STATE_TRANSITION');app=transition(app,'provisioning',context.subjectId);const tenantId=app.tenantId??crypto.randomUUID();const updatedApp=transition(saveApplication({...app,tenantId}),'onboarding',context.subjectId);saveApplication(updatedApp);const createdAt=now();const request:ProvisioningRequestView={id:crypto.randomUUID(),applicationId,tenantId,status:'completed',currentStep:'onboarding_checklist_created',attempts:1,createdAt,updatedAt:createdAt};provisioning.set(request.id,request);platformOrganizations.set(applicationId,{applicationId,tenantId,provisioningRequestId:request.id,legalName:updatedApp.organizationName,organizationNumber:updatedApp.organizationNumber,organizationType:updatedApp.organizationType,applicationStatus:updatedApp.status,tenantStatus:'onboarding',provisioningStatus:request.status,...(request.currentStep?{currentStep:request.currentStep}:{}),primaryHostname:`${updatedApp.organizationName.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'').slice(0,40)||'organisation'}.kommunsign.se`,domainReady:true,primaryAdminEmail:updatedApp.primaryEmail,primaryAdminName:updatedApp.primaryContactName,primaryAdminTitle:updatedApp.primaryContactTitle,createdAt:updatedApp.createdAt});addAudit(applicationId,context.subjectId,'tenant.provisioning.completed',{requestId:request.id,tenantId});return request;});},
