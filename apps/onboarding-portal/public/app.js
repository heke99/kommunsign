...
$('#message-form').addEventListener('submit',async(event)=>{event.preventDefault();const body=new FormData(event.currentTarget).get('body');try{await api(`/v1/onboarding/applications/${state.applicationId}/messages`,{method:'POST',mutating:true,body:{body}});event.currentTarget.reset();notice('Meddelandet skickades.');await refresh();}catch(error){notice(`Meddelandet kunde inte skickas: ${error.message}`);}});
